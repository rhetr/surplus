...
import subprocess
import signal
from PyQt6.QtCore import QUrl, Qt, QMimeData, pyqtSignal

from .BaseList import BaseList, BaseListItem
...
class FileList(BaseList):
    sample_playing = pyqtSignal(bool)
    ...
    def _keyEvents(self, event):
        if event.key() == Qt.Key.Key_Escape:
            ...
            if (
                    os.path.abspath(self.current_item.text)
                    in config.settings['Recent']
                    ):
                config.settings['Recent'].remove(
                        os.path.abspath(self.current_item.text)
                        )
    ...
    def activatePressed(self, item):
        if self.select_flag:
            ...
            if self.current_item.is_single and self.playback_enabled:
                self.current_item.playSample()
            else:
                self.updateList(self.current_item.text)
    ...
    def updateList(self, dest):
        ...
        if dest == "Recent":
            self.cwd_dirs = ['..']
            self.cwd_items = config.settings['Recent'][::-1]
            self.drawContents()
            ...
            index = self.cwd_dirs.index(prev_folder) if prev_folder else 1
    ...
    def searchText(self, text):
        '''
        uses find, grep and awk to search
        ...
        cmd = (
                "find '{}' | grep -vE '\.asd|\.pf' | grep -E '{}'| awk {}"
                .format(
                    os.getcwd(),
                    audio_only,
                    ignore_case
